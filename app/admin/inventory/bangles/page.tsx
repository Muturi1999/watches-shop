  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold mb-2">Bangles</h1>
          <p className="text-muted-foreground">{filteredProducts.length} products</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button onClick={() => { resetForm(); setIsDialogOpen(true); }}>
              <Plus className="h-4 w-4 mr-2" />Add Bangle
            </Button>
          </DialogTrigger>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>{editingProduct ? 'Edit' : 'Add'} Bangle</DialogTitle></DialogHeader>
      ...
      {isLoading ? (
        <Card className="p-12 text-center">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-muted-foreground">Loading bangles...</p>
          </div>
        </Card>
      ) : (
        <div className="space-y-3">
          ...
          {filteredProducts.length === 0 && <Card className="p-12 text-center"><h3 className="text-lg font-semibold mb-2">No bangles found</h3><p className="text-muted-foreground">Try adjusting your filters or add new products</p></Card>}
        </div>
      )}
    </div>
  )
