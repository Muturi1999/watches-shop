    ...
    try {
      ...
      if (stored) {
        ...
        const filtered = allProducts.filter((p: any) => 
          p.category === CATEGORY || 
          (p.category.toLowerCase().includes('accessor') && !p.category.toLowerCase().includes('watch'))
        )
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>{editingProduct ? 'Edit' : 'Add'} Accessory</DialogTitle></DialogHeader>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          ...
          {filteredProducts.length === 0 && <Card className="p-12 text-center"><h3 className="text-lg font-semibold mb-2">No accessories found</h3><p className="text-muted-foreground">Try adjusting your filters or add new products</p></Card>}
        </div>
      )}
    </div>
  )
